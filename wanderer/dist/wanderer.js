...
var require_wanderer_image = __commonJS2({
  "../plugins/image/dist/wanderer-image.js"(exports2, module2) {
    ...
    var src_exports3 = {};
    __export3(src_exports3, {
      ImagePlugin: () => ImagePlugin2
    });
    module2.exports = __toCommonJS3(src_exports3);
    var import_upath8 = __toESM3(require_upath3());
    var import_fs22 = __toESM3(require("fs"));
    var import_dayjs = __toESM3(require_dayjs_min2());
    var import_fs7 = __toESM3(require("fs"));
    var import_path4 = __toESM3(require("path"));
    var import_child_process = require("child_process");
    var processImageGM = (inputFilePath, targetFilePath, config2, callback) => {
      config2 = config2 || {};
      const targetFileDir = import_path4.default.dirname(targetFilePath);
      const ext = import_path4.default.extname(inputFilePath).toLowerCase();
    ...
    var processImage = (inputFilePath, targetFilePath, config2 = {}) => {
      const inputFile = import_path4.default.basename(inputFilePath);
...
var require_upath2 = __commonJS2({
  "../node_modules/.pnpm/upath@1.2.0/node_modules/upath/build/code/upath.js"(exports2) {
    ...
    for (propName in path9) {
      propValue = path9[propName];
      if (isFunction(propValue)) {
        upath[propName] = function(propName2) {
          return function() {
            var args, result;
            ...
            result = path9[propName2].apply(path9, args);
            if (isString(result)) {
              return toUnix(result);
            } else {
              return result;
      ...
      return function() {
        ...
        const parser = (() => {
          ...
          var tokenizer = function(path9) {
            ...
            var scanRegex = function() {
              ...
              while (position < length) {
                var currentChar = path9.charAt(position);
                if (currentChar === "/" && path9.charAt(position - 1) !== "\\" && depth === 0) {
                  pattern = path9.substring(start, position);
                  if (pattern === "") {
                    throw {
                      code: "S0301",
                      stack: new Error().stack,
                  ...
                  position++;
                  currentChar = path9.charAt(position);
                  start = position;
                  while (currentChar === "i" || currentChar === "m") {
                    position++;
                    currentChar = path9.charAt(position);
                  }
                  flags = path9.substring(start, position) + "g";
                  return new RegExp(pattern, flags);
                }
                if ((currentChar === "(" || currentChar === "[" || currentChar === "{") && path9.charAt(position - 1) !== "\\") {
                  depth++;
                }
                if ((currentChar === ")" || currentChar === "]" || currentChar === "}") && path9.charAt(position - 1) !== "\\") {
                  depth--;
            ...
            var next = function(prefix) {
              ...
              if (currentChar === "/" && path9.charAt(position + 1) === "*") {
                var commentStart = position;
                position += 2;
                currentChar = path9.charAt(position);
                while (!(currentChar === "*" && path9.charAt(position + 1) === "/")) {
                  currentChar = path9.charAt(++position);
                  if (position >= length) {
                    throw {
                      code: "S0106",
                ...
                while (position < length) {
                  ...
                  if (currentChar === "\\") {
                    ...
                    if (Object.prototype.hasOwnProperty.call(escapes2, currentChar)) {
                      ...
                      var octets = path9.substr(position + 1, 4);
              ...
              var match = numregex.exec(path9.substring(position));
              if (match !== null) {
              ...
              if (currentChar === "`") {
                position++;
                var end = path9.indexOf("`", position);
                if (end !== -1) {
                  name = path9.substring(position, end);
                  position = end + 1;
              ...
              for (; ; ) {
                ...
                if (i === length || " 	\n\r\v".indexOf(ch) > -1 || Object.prototype.hasOwnProperty.call(operators, ch)) {
                  if (path9.charAt(position) === "$") {
                    name = path9.substring(position + 1, i);
                    position = i;
                    return create("variable", name);
                  } else {
                    name = path9.substring(position, i);
                    position = i;
                    switch (name) {
                      case "or":
                      case "in":
          ...
          var parser2 = function(source, recover) {
            ...
            var resolveAncestry = function(path9) {
              var index = path9.steps.length - 1;
              var laststep = path9.steps[index];
              ...
              for (var is = 0; is < slots.length; is++) {
                var slot = slots[is];
                index = path9.steps.length - 2;
                while (slot.level > 0) {
                  if (index < 0) {
                    if (typeof path9.seekingParent === "undefined") {
                      path9.seekingParent = [slot];
                    } else {
                      path9.seekingParent.push(slot);
                    }
                    break;
                  }
                  var step = path9.steps[index--];
                  while (index >= 0 && step.focus && path9.steps[index].focus) {
                    step = path9.steps[index--];
                  }
...
var loadPluginFromString = async (pluginPath, baseDir2) => {
  try {
    if (pluginPath.startsWith(".")) {
      pluginPath = import_upath3.default.resolve(baseDir2 || process.cwd(), pluginPath);
    }
...
var _readdirSyncRecursive = function(dirPath, _originalDirPath, arrayOfFiles = []) {
  ...
  files.forEach(function(file) {
    if (import_fs3.default.statSync(import_path2.default.join(dirPath, file)).isDirectory()) {
      arrayOfFiles = _readdirSyncRecursive(import_path2.default.join(dirPath, file), originalDirPath, arrayOfFiles);
    } else {
      arrayOfFiles.push(import_path2.default.normalize(import_path2.default.relative(originalDirPath, import_path2.default.join(dirPath, file))));
    }
  });
  return arrayOfFiles;
};
...
var pathUtil = __toESM2(require("path"), 1);
...
var streamFrontmatter = async (filename) => {
  let startToken = "";
  const fileStream = import_fs4.default.createReadStream(import_upath4.default.resolve(filename));
...
var getTextAfterFrontmatter = (filename) => {
  const text3 = import_fs4.default.readFileSync(import_upath4.default.resolve(filename), "utf-8");
  let startToken = "";
  if (text3.startsWith("---")) {
    startToken = "---";
...
var FileDB = class {
  ...
  queryDirectory(directory, opts = {}, alsoMatchSubdirectories) {
    const predicate = {
      key: "sourceDir",
      value: import_upath5.default.resolve(directory),
...
var FileCache = class {
  ...
  async generateConfig() {
    ...
    for (let file of allFiles) {
      const fullPath = import_upath6.default.resolve(this.siteInfo.contentDirectory, file);
      const dir = import_upath6.default.dirname(file);
      const ext = import_upath6.default.extname(file);
      const name = import_upath6.default.basename(file, ext);
      ...
      if (ext.toLocaleLowerCase() !== ".toml") {
        ...
        const contents = import_fs5.default.readFileSync(import_upath6.default.resolve(this.siteInfo.contentDirectory, file), "utf-8");
        if (name === `_`) {
  ...
  async generateFiles() {
    ...
    for (let file of allFiles) {
      const dir = import_upath6.default.dirname(file);
      const ext = import_upath6.default.extname(file);
      const rawname = import_upath6.default.basename(file, ext);
      ...
      const pathDir = dir || import_upath6.default.dirname(file);
      const pathEnd = config2.rename || name;
      let url2 = [pathDir === "." ? "" : pathDir, pathEnd === "index" ? "" : pathEnd].join("/");
      if (!url2.startsWith("/")) {
        url2 = "/" + url2;
      }
      const sourcePath = import_upath6.default.resolve(this.siteInfo.contentDirectory, file);
      const stats = import_fs5.default.statSync(sourcePath);
      const info = {
        ...
        ext,
        sourcePath,
        sourceDir: import_upath6.default.dirname(sourcePath),
        created: stats.birthtime,
        updated: stats.mtime,
        date: tempoString.date ?? stats.mtime
      };
      if (isText(sourcePath, import_fs5.default.readFileSync(sourcePath))) {
        const pageInfo = info;
  ...
  getConfigForFile(fileId, recursive = 2 /* ALL */) {
    const parts = fileId.split(import_upath6.default.sep);
    if (recursive === 0 /* NONE */) {
    ...
    if (recursive === 2 /* ALL */) {
      ...
      for (let part of parts) {
        currentPath = import_upath6.default.join(currentPath, part);
  ...
  getFileFromPath(sourcePath) {
    const query = {
      predicates: [
        {
          key: "sourcePath",
          value: import_upath6.default.resolve(sourcePath),
          modifier: "=" /* EQUALS */
        }
      ]
    };
  ...
  getFilesWithSimilarNames(file) {
    ...
    const sourceDir = import_upath6.default.dirname(file.sourcePath);
...
var Site = class {
  constructor(config2) {
    ...
    this.siteInfo = {
      ...config2,
      contentDirectory: import_upath7.default.resolve(config2.contentDirectory),
      frameDirectory: import_upath7.default.resolve(config2.frameDirectory),
      cacheDirectory: import_upath7.default.resolve(config2.cacheDirectory),
      buildDirectory: import_upath7.default.resolve(config2.buildDirectory)
    };
  ...
  get staticDir() {
    return import_upath7.default.resolve(this.siteInfo.frameDirectory, "static");
  }
  get buildDir() {
  ...
  async make() {
    ...
    for (let file of staticFiles) {
      const targetFile = import_upath7.default.resolve(this.buildDir, "static", file);
      if (!import_fs6.default.existsSync(import_upath7.default.dirname(targetFile))) {
        import_fs6.default.mkdirSync(import_upath7.default.dirname(targetFile), { recursive: true });
      }
      import_fs6.default.copyFileSync(import_upath7.default.resolve(this.staticDir, file), targetFile);
    }
...
var main = async () => {
  Logger.set(2 /* WARN */);
  const siteConfig = {
    contentDirectory: import_path3.default.resolve(__dirname, "..", "test", "content"),
    frameDirectory: import_path3.default.resolve(__dirname, "..", "test", "frame"),
    buildDirectory: import_path3.default.resolve(__dirname, "..", "build", "test"),
    cacheDirectory: import_path3.default.resolve(__dirname, "..", "build", ".cache"),
    siteConfiguration: {}
  };
  wanderer(siteConfig);
};
