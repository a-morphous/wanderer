...
var genLayout = (siteInfo, file) => {
  ...
  if (fs.existsSync(layoutPath)) {
    layoutText = fs.readFileSync(import_upath.default.resolve(siteInfo.frameDirectory, "layouts", layout + ".html")).toString();
  }
  return layoutText;
};
...
var render = (template, config, layer) => {
  ...
  for (const line of splits) {
    ...
    if (line.startsWith("import ")) {
      ...
      importedPartials[key2] = fs2.readFileSync(import_upath2.default.resolve(baseDir, partialPath), "utf-8");
      continue;
    }
    processedTemplate += line + "\n";
...
var MarkdownPlugin = class {
  ...
  build(opts, dryRun = false) {
    ...
    if (!fs3.existsSync(targetPath)) {
      fs3.mkdirSync(import_upath4.default.dirname(targetPath), { recursive: true });
    }
