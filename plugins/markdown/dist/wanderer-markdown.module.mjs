...
var hasOwnProperty2, protocolHref, protocolSrc;
...
var genLayout = (siteInfo, file) => {
  ...
  const layoutPath = path2.resolve(siteInfo.frameDirectory, "layouts", layout + ".html");
  let layoutText = "${o.content}";
  if (fs2.existsSync(layoutPath)) {
    layoutText = fs2.readFileSync(path2.resolve(siteInfo.frameDirectory, "layouts", layout + ".html")).toString();
  }
  return layoutText;
};
    ...
    for (let feedName of Object.keys(file.configuration.feeds)) {
      ...
      if (typeof rawQuery === "string") {
        if (rawQuery.startsWith(".")) {
          const directory = path2.resolve(file.sourceDir, rawQuery);
          feeds[feedName] = filePool.queryDirectory(directory, query, false);
          continue;
        }
        if (rawQuery.toLocaleLowerCase().startsWith("dir:")) {
          const directory = path2.resolve(file.sourceDir, rawQuery.slice(4));
          feeds[feedName] = filePool.queryDirectory(directory, query, false);
          continue;
        }
...
var MarkdownPlugin = class {
  ...
  build(opts, dryRun = false) {
    ...
    if (!fs3.existsSync(targetPath)) {
      fs3.mkdirSync(import_upath2.default.dirname(targetPath), { recursive: true });
    }
